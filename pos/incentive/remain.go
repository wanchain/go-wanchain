...
func addRemainIncentivePool(stateDb *state.StateDB, epochID uint64, remainValue *big.Int) {
	...
	hash := crypto.Keccak256Hash(postools.Uint64ToBytes((epochID/subsidyReductionInterval)+1), []byte(dictRemainPool))
...
func getRemainIncentivePool(stateDb *state.StateDB, epochID uint64) *big.Int {
	...
	hash := crypto.Keccak256Hash(postools.Uint64ToBytes(epochID/subsidyReductionInterval), []byte(dictRemainPool))
