...
func TestCalcGasLimit(t *testing.T) {
	...
	parent := types.NewBlock(header, nil, nil, nil, nil)
	for i, tc := range []struct {
		...
		// Increase
		if have, want := CalcGasLimit(parent, tc.pGasLimit, 2*tc.pGasLimit), tc.max; have != want {
			t.Errorf("test %d: have %d want <%d", i, have, want)
		}
		// Decrease
		if have, want := CalcGasLimit(parent, tc.pGasLimit, 0), tc.min; have != want {
			t.Errorf("test %d: have %d want >%d", i, have, want)
		}
		// Small decrease
		if have, want := CalcGasLimit(parent, tc.pGasLimit, tc.pGasLimit-1), tc.pGasLimit-1; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
		// Small increase
		if have, want := CalcGasLimit(parent, tc.pGasLimit, tc.pGasLimit+1), tc.pGasLimit+1; have != want {
		...
		// No change
		if have, want := CalcGasLimit(parent, tc.pGasLimit, tc.pGasLimit), tc.pGasLimit; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
	}
}
