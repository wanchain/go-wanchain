...
func (c *wanchainStampSC) buyStamp(in []byte, contract *Contract, evm *Interpreter) []byte {
	...
	add, err := AddOTAIfNotExit(evm.env.StateDB, contract.value, in)
	if err != nil || !add {
		return nil
	}
...
func (c *wanchainStampSC) verifyStamp(all []byte, contract *Contract, evm *Interpreter) []byte {
	...
	for i = 0; i < pubsLen; i++ {
		...
		//	return nil
		//}
		otaAXs = append(otaAXs, all[idx+1:idx+1+common.HashLength])
	...
	if !exit || balanceGet == nil || balanceGet.Cmp(common.Big0) == 0 {
		if err != nil {
			log.Warn("verify mix ota fail. err:", err.Error())
		}
		if unexit != nil {
			log.Warn("invalid mix ota:", common.ToHex(unexit))
		}
	...
	if err != nil || exit {
		...
		verifyRes := crypto.VerifyRingSign(txhashBytes, PublicKeySet, KeyImage, []*big.Int(W_random), []*big.Int(Q_random))
		if verifyRes {
			AddOTAImage(evm.env.StateDB, kix, balanceGet.Bytes())
...
func (c *wanCoinSC) refund(all []byte, contract *Contract, evm *Interpreter) []byte {
	...
	for i = 0; i < pubsLen; i++ {
		...
		otaAXs = append(otaAXs, x[1:1+common.HashLength])
	...
	exit, balanceGet, unexit, err := BatCheckOTAExit(evm.env.StateDB, otaAXs)
	if !exit || balanceGet == nil || balanceGet.Cmp(vb) != 0 {
		if err != nil {
			log.Warn("verify mix ota fail. err:", err.Error())
		}
		if unexit != nil {
	...
	if err != nil || exit {
		...
		verifyRes := crypto.VerifyRingSign(txhashBytes, PublicKeySet, KeyImage, []*big.Int(W_random), []*big.Int(Q_random))
		if verifyRes {
			AddOTAImage(evm.env.StateDB, kix, refundValBytes)
