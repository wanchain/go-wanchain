...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		log.Debug("handler, inserter begin..")
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockMsg:
		log.Debug("handleMsg, NewBlockMsg")
		...
		_, td := p.Head()
		log.Debug("before compare peer td", "trueTD", trueTD, "td", td)
		if trueTD.Cmp(td) > 0 {
			...
			log.Debug("before compare blockchain td", "trueTD", trueTD, "blockchain td", pm.blockchain.GetTd(currentBlock.Hash(), currentBlock.NumberU64()))
			if trueTD.Cmp(pm.blockchain.GetTd(currentBlock.Hash(), currentBlock.NumberU64())) > 0 {
				log.Debug("before synchromise goroutine")
				go pm.synchronise(p)
			}
		}
