...
func (c *Pluto) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header, isSlotVerify bool) error {
	...
	if len(header.Extra) == extraSeal {
		...
		proofStart := 1
		hc, ok := chain.(*core.HeaderChain)
		// is epoch genesis is generated by local
		bGenerate := false
		if !ok {
			bc,ok := chain.(*core.BlockChain)
			if !ok {
		...
		_, proofMeg, err := s.GetInfoFromHeadExtra(epochID, header.Extra[proofStart:len(header.Extra)-extraSeal])
		...
		if err != nil {
			...
			// TODO: is signer in genesis block
...
func (c *Pluto) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	if bc.GetHc().IsEpochFirstBlkNumber(epochId, number, nil) {
		...
		extra = make([]byte, len(buf) + 33 +extraSeal)
		extra[0] = 'g'
		copy(extra[1:], hash[:])
		copy(extra[33:], buf)
	} else {
	...
	err = c.verifySeal(chain, header, nil, true)
	if err != nil {
