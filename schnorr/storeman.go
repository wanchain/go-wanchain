...
import (
	...
	"github.com/wanchain/go-wanchain/schnorr/schnorrType"
...
func (sa *StoremanAPI) SignMpcTransaction(ctx context.Context, tx schnorrType.SendTxArgs) (hexutil.Bytes, error) {
	...
	if tx.To == nil ||
		...
		tx.Nonce == nil ||
		tx.ChainID == nil {
		return nil, schnorrType.ErrInvalidMpcTx
	}

//	log.SyslogInfo("SignMpcTransaction begin", "txInfo", tx.String())

	if len(sa.sm.peers) < MPCDegree*2 {
		return nil, schnorrType.ErrTooLessStoreman
	}
...
func (sa *StoremanAPI) AddValidMpcTx(ctx context.Context, tx schnorrType.SendTxArgs) error {
